...
import {
    ...
    ,InputGroup ,InputRightElement ,  Button, Container,
    Checkbox,
    HStack
...
const Authenticator = (props) =>{
    ...
    const account_type=props?.location?.state?.account_type
       ...
       const handleClickStripe = async (id) => {
        try {
          ...
          const response = await fetch(
            `https://api.dadavault.com/api/users/create-checkout-session?userId=${id}`,
            {
              method: "POST",
            } 
          );
    ...
    if(loader)
{
 ...
 <Box  mt={'32px'} pb={0} mb={0}>
                            <BioRymHeading color="#363535" fontSize='2.25rem' pt='1.5rem' mb='1rem'  fontWeight='700' >Check your authenticator</BioRymHeading>
                        ...
                        <Box className="Box-card" mt={'2rem'} mb={'auto'} p='2rem' bg='#fff' borderRadius='0.5rem' >
                            <Flex mb={'2rem'}>
                                <Link display='flex' alignItems={'center'} justifyContent={'center'} color={'#0F0EA7'} fontWeight={'500'} ><IoIosArrowBack /> <Text as='span'> Back to sign in</Text></Link>
                            </Flex>
                            <FormControl id="email">
                                ...
                                <Button  py='1.5rem' w='100%' my='2rem' borderRadius={'0px'} bg='#0F0EA7' color='#fff' _focus={{ bg: "#090864", }}  _hover={{ bg: "#0C0B86", }} _active={{ bg: "#090864", }}


                                         onClick={handle2fa}>Continue </Button>                            </FormControl>
                        </Box>
                       </>
                     )}
                        ...
                        {enabled &&(
                            <>
                              <Box  pb={0} mb={8}>
                                <Heading color="##363535" fontSize='1.25rem'  mb='0.5rem' lineHeight={'24px'} fontWeight='700' >2FA has been enabled</Heading>
                                <Text display={'flex'} alignItmes={'center'}justifyContent={'center'} pb={0} fontSize='16px' lineHeight={'24px'} fontWeight={'400'} color={'#797979'}>You will now receive a 6-digit code at your email every time you log in to make sure it’s you.</Text>
                            </Box>
                            <Box mb={'auto'}>
                                <Box mb={'2.5rem'}  className="Box-card" p='2rem' bg='#fff' borderRadius='0' boxShadow={'0px 1px 3px rgba(32, 31, 31, 0.1), 0px 1px 2px rgba(32, 31, 31, 0.06)'}  >


                                    <FormControl>
                                        <Checkbox defaultChecked={true} mb={'1rem'} size="md" colorScheme="blue" alignItems={'flex-start'}>
                                            <HStack spacing="1">
                                                <Box fontSize="2xl">
                                                </Box>

                                                <Box flex="1">
                                                    <Text lineHeight={'11px'} mb={1} textAlign={'left'} color={'#4D4C4C'} fontWeight={'500'} fontSize={'14px'}>Remember this device for 15 days</Text>
                                                    <Text textAlign={'left'} color={'#8F8F8F'}>We will not send a 2FA code to your email until 15 days from now.</Text>
                                                </Box>
                                        ...
                                        <Button
                                        onClick={checkout}
                                        py='1.5rem' w='100%' my='1rem' borderRadius={'0px'} bg='#0F0EA7' color='#fff' _focus={{ bg: "#090864", }}  _hover={{ bg: "#0C0B86", }} _active={{ bg: "#090864", }}
                        ...
                        {/*<Text color={'#201F1F'} fontWeight={'600'} fs={'16px'} my={'2.5rem'} >*/}
                        {/*    Get help with 2FA*/}
                        {/*</Text>*/}
                    </Container>
