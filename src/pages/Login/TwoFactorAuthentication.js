...
import {
    ...
    Box,
    ChakraProvider,
    Flex , Input , FormHelperText,Stack
    ,InputGroup ,InputRightElement ,  Button, Container, useColorModeValue as mode} from "@chakra-ui/react";
import {BioRymHeading} from '../../assets/StyledComponent/styeledComponent';
import WelcomeLogin from "../Login/WelcomeLogin";
import {IoIosArrowBack} from "react-icons/io";
import { FaMobileAlt, FaShieldAlt,FaEnvelope } from 'react-icons/fa'
...
const TwoFactorAuthentication = (props) =>{
    ...
    const email=props?.location?.state?.value?.email
    const name=props?.location?.state?.value?.name
    const userId=props?.location?.state?.value?._id

    const [loader,setLoader]=React.useState(false)

    const  account_type =  props?.location?.state?.value?.account_type

    useEffect(() => {
        if(account_type==='collector'){
        showCollectorVerificationToast()          
        }
         },[]);

         const showCollectorVerificationToast =async() => {
        ...
        const {data} = await server.post(
          ...
          if(data)
          {
            setLoader(false)
              ...
              navigate("/LoginAuthenticator", { state: { value: email,account_type,name,userId }})
             // navigate("/enabled2fa")
          }
          else
          {
                ...
                <Box as='section' bg={'#F7F7F7'} className='Login'>
                    <Flex height='100vh' overflowY={'scroll'} >
                        <Container display={'flex'} flexDirection={'column'}   minHeight='100vh'>
                            ...
                            <Box mt={'2rem'} pb={0} mb={0} textAlign={'center'}>
                                <Heading color="#201F1F"  fontSize='1.25rem' pt='1.5rem' mb='1rem'  fontWeight='700' >Two-factor authentication (2FA)</Heading>
                                <Text display={'flex'} alignItmes={'center'} marginBottom={'2rem'} justifyContent={'center'} pb={0} fontSize='16px' fontWeight={'400'} color={'#797979'}> Secure your account by adding another challenge to logins.</Text>
                            ...
                            <Box mb={'auto'}>
                                    ...
                                    <FormControl>
                                        ...
                                        >
                                            <Stack direction={{ base: 'column', sm: 'row' }} spacing="4" align="flex-start" flex="1">
                                                <Box aria-hidden fontSize="2xl" pt="1" color="#8F8F8F">
                                                    <FaEnvelope />
                                                </Box>
                                                <Box flex="1">
                                                    <Text fontWeight={'700'} color={'#000'} textAlign={'left'}>
                                                        Authenticator
                                                    </Text>
                                                    <Text fontWeight={'400'} color={'#797979'} textAlign={'left'}>
                                                        Enter a code generated by your authenticator app to confirm it’s you.
                                                    </Text>

                                                </Box>
                                            </Stack>
                                            <Button bg={'#0F0EA7'} borderRadius={'0'} color={'#fff'} _focus={{ bg: "#090864", }}  _hover={{ bg: "#0C0B86", }} _active={{ bg: "#090864", }}
                                                    onClick={handle2fa}>
                                                Enable
                                            </Button>
                                        </Stack>
                                ...
                                <Box textAlign={'center'}>
                                    <Button onClick={skip2FA} color={'#201F1F'} TextAlign={'center'} fontWeight={'600'} variant='ghost'  borderRadius={'0px'}>Skip for Now </Button>
                                </Box>
