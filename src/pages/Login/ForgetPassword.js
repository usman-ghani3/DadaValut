...
import {Link as ReachLink, navigate, Redirect, redirectTo} from '@reach/router';
...
const ForgetPassword = ({ForgetPassword}) =>{
   const toast =useToast()
    ...
    const [checkForgetStatus,setCheckForgetStatus]=React.useState(false)
    
    const handleClickConfirm = () => setShow(!showConfirm)
    useEffect(() => {
        localStorage.clear()
        validateUser() 
       },[]);
    ...
    const validateUser =async() => {
        ...
        const {data} = await server.get(
            `users/VerifyForgot/${ForgetPassword}`,
            { 
...
if (!validUser)
{
  return (
    <Box>
      <text>invalid or Broken Forget Password Link </text>
    </Box>
  )
}
else if (validUser)
                ...
                <Box as='section' bg={'#F7F7F7'} className='Login'>
                    <Flex height='100vh' overflowY={'scroll'} >
                        <Container display={'flex'} flexDirection={'column'}  align='center' justifyContent='center'  direction="column"  minHeight='100vh'>
                            ...
                            <Box mt={'4rem'} pb={0} mb={0}>
                               {!checkForgetStatus?
                               <>
                                <BioRymHeading color="#201F1F" fontSize='2.25rem'  mb='1rem'  fontWeight='700' lineHeight={'43.6px'} >Reset your password</BioRymHeading>
                                <Text display={'flex'} alignItmes={'center'} justifyContent={'center'} pb={0}  mb='2rem' fontSize='16px' fontWeight={'400'} lineHeight={'24px'} color={'#797979'} >Enter your new password</Text>
                               </>
                               :
                               <>
                               <BioRymHeading color="#201F1F" fontSize='2.25rem'  mb='1rem'  fontWeight='700' lineHeight={'43.6px'} >You reset your password</BioRymHeading>
                                <Text display={'flex'} alignItmes={'center'} justifyContent={'center'} pb={0}  mb='2rem' fontSize='16px' fontWeight={'400'} color={'#797979'} lineHeight={'24px'} >You will need to sign in again with your new password</Text>
                                </>
}

                            </Box>
                            <Box mb={'auto'} className="Box-card" py='2rem' px={'2.5rem'} bg='#fff'  borderRadius='0' boxShadow={'0px 1px 3px rgba(32, 31, 31, 0.1), 0px 1px 2px rgba(32, 31, 31, 0.06)'} >
                                {!checkForgetStatus?
                                <>
                                <FormControl>
                        ...
                        border={(formikProps.errors.password && formikProps.touched.password)?'2px solid #DD2922': '1px solid #D2D2D2'}
                                        ...
                                        />
                                        <InputRightElement >
                                            <Button _focus={{ bg: "transparent", }}  _hover={{ bg: "transparent", }} _active={{ bg: "transparent", }} bg='transparent' h="1.75rem" size="sm" px={'0px'} onClick={handleClick} color={'#636262'}>
                                                {show ?  <IoMdEye  fontSize={'20px'}/> : <IoMdEyeOff  fontSize={'20px'}/> }
                                            </Button>
                                            {/*<Button _focus={{ bg: "transparent", }}  _hover={{ bg: "transparent", }} _active={{ bg: "transparent", }} bg='transparent' h="1.75rem" size="sm"  color={'#636262'}  onClick={handleClick}>*/}
          ...
          {formikProps.errors.password && formikProps.touched.password && (
               ...
               {formikProps.errors.password}
                                    ...
                                    <Button  py='1.5rem' w='100%' mt='2rem' borderRadius={'0px'} bg='#0F0EA7' color='#fff'_focus={{ bg: "#090864", }}  _hover={{ bg: "#0C0B86", }} _active={{ bg: "#090864", }}
                                             ...
                                             isDisabled={!formikProps?.values?.password?true:false}
                                             onClick={formikProps.handleSubmit}>Reset Password</Button>


                                </FormControl>
                                </>
                                :
                                <Button  py='1.5rem' w='100%' borderRadius={'0px'} bg='#0F0EA7' color='#fff' _focus={{ bg: "#090864", }}  _hover={{ bg: "#0C0B86", }} _active={{ bg: "#090864", }}
