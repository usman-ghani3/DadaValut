...
import {
    ...
    InputRightElement,
    Spacer,
    Checkbox, Text, Link, Divider, useColorModeValue
...
import {IoIosArrowBack, IoIosLock, IoMdEye, IoMdEyeOff} from "react-icons/io";
...
const stripePromise = loadStripe(
  "pk_test_51KPWoHAEBB4Crcloj2BnmpGudRHh8oLwLvFGvphnSISWa0yMf6Q4TjXtEcy7Wq3sShKv2iFTTzbEOdzLmc6eBfzH00ufhSFYEw"
    ...
    const GalleryProfile =({InvitationCode})=>{
      const [validInvitation, setValidInvitation] = React.useState(true);
      const [accountType, setAccountType] = React.useState('');
      const [invitedId, setInvitedId] = React.useState(true);
       ...
       const validateInvitation =async() => {
         console.log(InvitationCode)
           ...
           const {data} = await server.get(
               `users/createAccount/${InvitationCode}`,
               {
                 headers: {
  ...
  
    {
      ...
      bodyFormData.append(`account_type`, accountType);
        bodyFormData.append('password',values?.password );
        bodyFormData.append('email',email );
        bodyFormData.append('gallery_signup_step',1 );
        try  { 
          setLoader(true)
            ...
            // handleClickStripe(data?._id)
               navigate("/2fa", { state: { value: data } })
    ...
    const handleClickStripe = async (id) => {
      try {
        ...
        const response = await fetch(
          `http://localhost:8001/api/users/create-checkout-session?userId=${id}`,
          {
            method: "POST",
          } 
        );
        ...
        console.log("before result === ", session);
...
if (!validInvitation)
{
  return (
    <Box>
      <text>
        Invalid or broken invitation link.
      </text>
    </Box>
  )
}
...
<>
             ...
             <Box as='section' bg={'#F7F7F7'} className='Login'>
                 <Flex  overflowY={'scroll'} pb={'240px'} >
                     <Container display={'flex'} flexDirection={'column'}  align='center'   direction="column"  minHeight='100vh'>
                         <Flex mx='auto'  pt={'4rem'} pb={'6rem'}>
                         ...
                         <Box  pb={0} mb={0}>
                             <Heading color="#363535" fontSize='1.5rem'  mb='0.5rem'  fontWeight='700' lineHeight={'32px'} >You’re approved for DadaVault</Heading>
                             <Text display={'flex'} alignItmes={'center'} marginBottom={'2rem'} justifyContent={'center'} lineHeight={'24px'} pb={0}  fontSize='16px' fontWeight={'400'} color={'#797979'}>Just a few more steps to set up your DadaVault account.</Text>
                         </Box>
                         <Box   mb={5} className="Box-card" p='2rem' bg='#fff' borderRadius='0' boxShadow={'0px 1px 3px rgba(32, 31, 31, 0.1), 0px 1px 2px rgba(32, 31, 31, 0.06)'}  >
                             <FormControl>
                                 <Text textAlign={'left'} color='#636262' fontWeight='500' mb={2} lineHeight={'24px'} >Email Address</Text>
                                 <InputGroup size="md" minH={'2rem'}  mb={'0.5rem'}>
                                     ...
                                     <InputRightElement width="fit-content">
                                         <Button _focus={{ bg: "transparent", }} px={'15px'}  _hover={{ bg: "transparent", }} _active={{ bg: "transparent", }} bg='transparent' h="1.75rem" size="sm" >
                                             <IoIosLock fontSize={'18px'} color={'#B3B3B3'}/>
                                         </Button>
                                     </InputRightElement>
                                 </InputGroup>
                                 <Text fontSize={'14px'} fontWeight={'400'} marginBottom={'2rem'} color={'#8F8F8F'} textAlign={'left'} >Contact DadaVault if you need to change this.</Text>
                                 <Text textAlign={'left'} color='#636262' fontWeight='500'  lineHeight={'24px'} mb={2}>Password</Text>
                                 <InputGroup size="md" mb={2}>
                                     <Input
                                         borderRadius={'0'}
                                         borderColor={(formikProps.errors.password && formikProps.touched.password)?'#DD2922': ' #D2D2D2'}
                                         border={(formikProps.errors.password && formikProps.touched.password)?'2px solid #DD2922': '1px solid #D2D2D2'}
                                         pr="4.5rem"
                                     ...
                                     <InputRightElement width="fit-content">
                                         <Button _focus={{ bg: "transparent", }}  px={'15px'} _hover={{ bg: "transparent", }} _active={{ bg: "transparent", }} bg='transparent' h="1.75rem" size="sm" onClick={handleClick} color={'#b3b3b2'}>
                                             {show ? <IoMdEye fontSize={'20px'}/> : <IoMdEyeOff fontSize={'20px'}/>}
                                         </Button>
                                     </InputRightElement>
                                 </InputGroup>
                                 <Box mb={6}>
                                 <Text fontSize={'14px'} fontWeight={'400'} color={'#8F8F8F'} textAlign={'left'} mb={2}>Minimum 8 characters</Text>
                                 <div>
                                  {formikProps.errors.password && formikProps.touched.password && (
                                       <Alert status='error' color={'#DD2922'} fontWeight={'400'} fontSize={'14px'} lineheight={'20px'} ps={0}>
                                 ...
                                 <FormLabel textAlign={'left'} color='#636262' fontWeight='500' fontSize={'16px'} line >Disclosures</FormLabel>
                                 <Box mb={'32px'}>
                                 <Flex >
                                     <Box mb={'8px'}>
                                         <Text color='#636262' fontWeight='400' >
                                             <Checkbox display={'flex'} alugnItems={'baseline'} variant="red" color='#636262'
                                              value={formikProps.values.checkbox}
                                         onChange={formikProps.handleChange("checkbox")}
                                         onBlur={formikProps.handleBlur("checkbox")}
                                         error={
                                           formikProps.errors.checkbox && formikProps.touched.checkbox
                                             ? true
                                             : false
                                         }
                                          textAlign={'left'}
                                             >
                                                <Text pl={2}>I have read and accepted the DadaVault <Link textDecoration={'underline'}>Privacy Policy</Link> and <Link textDecoration={'underline'}>Terms & Conditions</Link>. </Text></Checkbox></Text>
                                     </Box>
          ...
          {formikProps.errors.checkbox && formikProps.touched.checkbox && (
               ...
               {formikProps.errors.checkbox}
                                 ...
                                 <Divider  mb={'1.5rem'} borderColor={'#BCBCBC'} />
                                 <Button display={'flex'} alignItems={'center'} justifyContent={'flex-end'} ml={'auto'}  maxW={'145px'} py='1.5rem' w='100%' my='1rem' borderRadius={'0px'} bg='#0F0EA7' color='#fff' _focus={{ bg: "#090864", }}  _hover={{ bg: "#0C0B86", }} _active={{ bg: "#090864", }}

                                        isDisabled={(formikProps?.values?.password && formikProps?.values?.checkbox)?false:true}
                                         onClick={formikProps.handleSubmit}>Create account</Button>
                             </FormControl>
                         </Box>
             ...
             :
             <Box height='100vh' display='flex'>             <Spinner
                                            thickness='4px'
                                            speed='0.65s'
                                            emptyColor='gray.200'
                                            color='blue.500'
